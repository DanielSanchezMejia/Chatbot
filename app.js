		...
		if(result){
			...
			if(fulfillment && fulfillment.speech && fulfillment.speech.length>0){
				...
				switch(action){
					...
					checkAccount(senderID, "balance");		//sendTextMessage(senderID, 'get account balance');
					break;
					case 'account.movement':
					checkAccount(senderID, "balance");	//sendTextMessage(senderID, 'get account movement');
...
function checkAccount(senderID,messages){
    ...
    if (!error && response.statusCode == 200) {
		...
		if(meme && meme.images && meme.fixed_width){
